/*******************************************************************
  SUZUKI NJS SOEKARNO HATTA – script.js (versi fix + SEO friendly)
  ...
  products.forEach((product) => {
    ...
    card.innerHTML = `
      ...
      <div class="product-image">
        ${
          product.image
            ? `<img src="${product.image}" alt="${product.name} Bandung & Cimahi" loading="lazy" />`
            : "Gambar Mobil"
        }
      </div>
...
function renderProductModalHeader() {
  ...
  const taglineEl = document.getElementById("modal-product-tagline");
  ...
  if (taglineEl) {
    taglineEl.textContent =
      currentProduct.category === "passenger"
        ? "Mobil penumpang Suzuki untuk keluarga di Bandung & Cimahi."
        : "Kendaraan niaga Suzuki untuk kebutuhan usaha di Bandung & Cimahi.";
  }

  if (imgEl) {
    if (currentProduct.image) {
      imgEl.src = currentProduct.image;
      imgEl.alt = `${currentProduct.name} – Dealer Suzuki NJS Cimahi Bandung`;
    } else {
      imgEl.removeAttribute("src");
      imgEl.alt = `Mobil Suzuki ${currentProduct.name}`;
    }
  }
}

/* helper: dapatkan / buat container di modal-right */
function getOrCreateInModalRight(id, className) {
  let el = document.getElementById(id);
  if (el) return el;

  const parent = document.querySelector("#product-modal .modal-right");
  ...
  const priceBlock = parent.querySelector(".modal-price-block");
  if (priceBlock) {
    parent.insertBefore(el, priceBlock);
  } else {
...
function renderVariantButtons() {
  const container =
    getOrCreateInModalRight("variant-buttons", "variant-buttons") ||
    document.getElementById("variant-buttons");
...
function renderColorButtons(variant) {
  let container =
    getOrCreateInModalRight("color-buttons", "color-buttons") ||
...
function updateProductModalView() {
  ...
  const quickList = document.getElementById("modal-specs-quick-list");
  if (quickList) {
    quickList.innerHTML = "";
...
function renderDeliveryGallery() {
  ...
  deliveryImages.forEach((item) => {
    ...
    div.innerHTML = `
      <img src="${item.image}" 
           alt="${item.caption || "Serah terima mobil Suzuki di Bandung & Cimahi"}" 
           loading="lazy">
    `;
...
function openDeliveryModal(image, caption) {
  ...
  imgEl.src = image;
  imgEl.alt = caption || "Serah terima mobil Suzuki di Bandung & Cimahi";
  overlay.classList.remove("hidden");
}
